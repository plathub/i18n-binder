...
public class I18nFacadeTest
{
	...
	public void testSilentlyIgnoringExceptionTranslationMapWithMissingKey()
	{
		...
		String[] keys = ArrayUtils.add(i18nFacade.I18n._673numericalTest.forLocale(locale).translator().allPropertyKeys(), 0, "missingKey");
		Map<String, String> map = i18nFacade.I18n._673numericalTest.translator().translate(keys);
		assertEquals(Arrays.asList("missingKey", "my.property.key1", "my.property.key3"), new ArrayList<String>(map.keySet()));
		assertEquals(Arrays.asList(null, "value {0} and {1}", "value3 with {arbitrary} replacement"), new ArrayList<String>(map.values()));
	}

	// requires @FixMethodOrder(MethodSorters.NAME_ASCENDING)
	// zzz -> Must be the last test, because the static resourceBasedTranslator is changed, which would affect all tests afterwards
	@Test
	public void zzz_testMockingTheFacade()
	{
